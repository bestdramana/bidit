...
.controller('BidCtrl', function($scope, $ionicConfig) {
  ...
  $scope.showFuture = function showFuture(hours) {
  	if(hours == 0) {
			$scope.futureDate = '----';
  	} else {
			var now = new Date();
			var newDate = new Date(now.getTime() + (hours*1000*60*60));
			var d = newDate.getDate();
			var m = newDate.getMonth()+1; //January is 0!
...
.controller('ShowBidCtrl', function($scope, $stateParams, $http, FeedList, $q, $ionicLoading, BookMarkService) {
	...
	var getDate = function (duration) {
					var now = new Date();
					var newDate = new Date(now.getTime() + (duration*1000*60*60));
					var d = newDate.getDate();
					var m = newDate.getMonth()+1; //January is 0!
		...
		$http.get('feeds-bids.json').success(function(response) {
			$scope.bid = _.find(response, {id: bidId });
		...
		cordova.plugins.email.isAvailable(
			function (isAvailable) {
				...
				cordova.plugins.email.open({
					...
					subject: 'Greetings',
					body:    'How are you? Nice greetings from bit-it'
				});
			}
		);
	};
